...
CREATE TABLE `users` (
  ...
  `survey_completed_at` DATETIME(3) DEFAULT NULL,
  `survey_age_range` VARCHAR(32) DEFAULT NULL,
  `survey_source` VARCHAR(32) DEFAULT NULL,
  `survey_interests` JSON DEFAULT NULL,
...
INSERT INTO `categories` (`user_id`, `section`, `code`, `name`, `is_system`, `sort_order`) VALUES
  ...
  (0, 'system', 'untagged',  '无标签', 1, 55),
  (0, 'system', 'annotated', '批注', 1, 60),
